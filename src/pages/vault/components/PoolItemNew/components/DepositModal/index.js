...
export default function DepositModal(props) {
  ...
  const { convexVaultsIFOContract } = useUserPoolInfo(info, refreshTrigger)
  ...
  const selectTokenInfo = useToken(selectToken.address, refreshTrigger, 'vaultIFO')
  ...
  useDebounceEffect(
    async () => {
      ...
      if (canDeposit && !cBN(depositAmountInWei).isZero() && !isSelfLp) {
        ...
        const shares = await convexVaultsIFOContract.methods
  ...
  const handleApprove = async () => {
    ...
    try {
      const apiCall = tokenContract.methods.approve(
        config.contracts.concentratorIFOVault,
  ...
  const handleZapDeposit = async () => {
    ...
    try {
      const apiCall = convexVaultsIFOContract.methods.zapAndDeposit(info.id, selectToken.address, depositAmountInWei, minOut)
